...
async def extract_biomarkers(
    ...
    merged = _merge_biomarcadores(row.biomarcadores or [], guardados)
    row.biomarcadores = merged
    await session.flush()
    await session.refresh(row)

    log.info(
        "lab_upload.extracted user_id=%s guardados=%d advertencias=%d",
        user.id, len(guardados), len(advertencias),
    )
