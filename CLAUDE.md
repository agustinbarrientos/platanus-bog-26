...
- Routers reales: `auth`, `profile` (`/me`), `health_context`, `lab_upload` (extracción con Claude), `biological_age` (`/phenoage`, `/montecarlo`), `report` (`/reporte` JSON + `/reporte.pdf`: el reporte de salud orientativo de [docs/MOIRAI_REPORTE_SPEC.md](docs/MOIRAI_REPORTE_SPEC.md) construido en `app/report/` desde el motor real — rangos de referencia en `health_metrics/reference_ranges.py`, ejes + triage por reglas en `health_metrics/ejes.py`, literatura por palanca en `health_metrics/evidencia.py`; PDF con reportlab y fuentes en `app/report/fonts/`; `tests/test_report.py` pasa una lista de palabras prohibidas sobre todo el texto: nunca nombra enfermedades ni prescribe), `health_chat` (`/chat`: Moirai con RAG léxico en `app/chat_rag/` — fragmentos por tema de los datos guardados + el `resultado` que manda la app + base de conocimiento derivada de las tablas del motor; BM25 + sinónimos en español, presupuesto de tokens, herramienta `buscar_mis_datos`; modelo en `CHAT_MODEL`), `voice` (`/me/voice/estado|tts|stt`: proxy autenticado a ElevenLabs — la API key vive en el servidor, nunca en el APK; `app/voice_text.py` normaliza el texto para que suene bien en es-CO antes de sintetizar). La voz del chat (`app/chat_rag/prompt.py`) es cálida, sencilla y humana —la persona debe sentirse escuchada y recibir respuesta a lo que preguntó—; el registro sale de `demografia.perfil_conocimiento` (`general|curioso|profesional`, se pregunta en el onboarding y la app lo manda en cada turno) y solo se vuelve técnica cuando la persona lo pide explícitamente. Cualquier cambio en un router se refleja en `API.md` en el mismo cambio.
- Esquema SQL en `schema.sql` (`scripts/apply_schema.py`). Secretos: `DATABASE_URL`, `ANTHROPIC_API_KEY`, `ELEVENLABS_API_KEY` + `ELEVENLABS_VOICE_ID` (env en Render; sin las dos últimas `/me/voice/*` responde 503 y el resto de la API sigue igual).
- Tests: `pytest` desde `apps/backend` (`pytest.ini` pone `pythonpath=.`): `tests/test_chat_rag.py` (retriever/documentos/prompt), `tests/test_health_chat_router.py` (el endpoint con DB y Anthropic falsos), `tests/test_report.py` (reporte: reglas, builder sobre el motor, barreras de lenguaje, PDF, endpoints) y `tests/test_voice.py` (normalización para voz + los tres endpoints con ElevenLabs falseado vía `httpx.MockTransport`) — sin red ni base de datos. Un test: `pytest tests/test_chat_rag.py::test_x`.
...
- `data/` — `api/api_client.dart` (única puerta al backend, `Authorization: Bearer` del token propio; 401 → cierra sesión) + `api/token_store.dart` (flutter_secure_storage), `models/` (espejo de spec §3/§8, `/me`, chat), `repositories/` (auth backend, perfil + health-context, exámenes vía `/biomarkers/extract`, simulación = `/phenoage` + `/montecarlo` adaptados a la forma de la spec §8, chat, voz (`/me/voice`, MP3 cacheado por texto), wearables con `health`), `mock/mock_engine.dart` (port en Dart de las 3 capas para demo sin backend; `test/mock_engine_test.dart` valida los invariantes de la spec §9).
- `features/<flujo>/` — pantallas: `auth`, `onboarding`, `exams`, `simulation`, `future`, `levers`, `backing`, `profile`, `report` ("Tu reporte", `/reporte`: muestra el JSON de `/reporte` —foto de hoy, ejes, recomendaciones con evidencia, ranking, con quién consultar, qué medir— y descarga/comparte el PDF de `/reporte.pdf` con `path_provider` + `share_plus`; se abre desde la tarjeta de "Tu futuro" y desde Perfil), `chat` ("Pregúntame": agente del backend con RAG; manda el `resultado` compacto + `enfoque`, muestra `fuentes` bajo cada respuesta; con voz — altavoz por respuesta, interruptor "leer en voz alta" y micrófono en el composer, en `voz_controller.dart`, con caída silenciosa a `flutter_tts` cuando el backend no puede hablar; es pestaña del bottom nav (`/preguntame`, `ChatScreen(enPestana: true)`) y se abre enfocado a pantalla completa con `Routes.chatCon(enfoque:, pregunta:)` (`/chat`) desde el detalle de una palanca y desde "Por qué"), `shell` (bottom nav Futuro · Simular · Pregúntame · Perfil; Respaldo ya no es pestaña: se abre con push desde el pill discreto de la esquina superior derecha de "Tu futuro").
...
3. **Monte Carlo**: corre la capa 2 N veces con ruido → mediana/P10/P90 por año. Biomarcadores imputados se **muestrean** de su dispersión poblacional (más incertidumbre ⇒ banda más ancha desde el año 0; medir angosta). Futuros **pareados** (misma semilla, mismos arranques, misma respuesta individual) ⇒ años ganados = distribución de la diferencia trayectoria a trayectoria. Luego barrido de combinaciones de 1–3 intervenciones rankeadas por años ganados / esfuerzo, y SHAP solo sobre el estado basal (contribución de cada biomarcador medido vs la mediana de referencia; de cada hábito a 10 años).

En el backend real esto vive en `app/health_metrics/` (`phenoage.py`, `interventions.py` (coeficientes + palancas + brechas por hábito), `evolution.py` (la regla de deriva), `montecarlo.py`, `nhanes_reference.py` (tabla de medianas calibrada + dispersión poblacional)) y se expone como `POST /me/health-context/phenoage` (edad biológica hoy + `contribuciones` + `percentil_poblacional`) + `POST /me/health-context/montecarlo` (lee los `habitos` guardados: línea base personal, solo palancas que aplican, combinaciones de 2–3; devuelve `curva` por año, `anios_ganados` pareados con rango, `pct_futuros_que_mejoran`, `muestra_trayectorias`, `valor_de_informacion`, `contribuciones_habitos`) + `GET /engine/catalogo` (constantes del motor, sin auth). Las 6 palancas: ejercicio, dieta mediterránea, tabaco, sueño, estrés, alcohol; los efectos sobre hs-CRP son proporcionales al valor actual. Lo único que la app sigue aproximando en local es la adherencia (ver API_CONTRACT.md). La spec §3/§8 sigue siendo la forma objetivo del resultado; validar cada capa con el caso de prueba de §9 (`pytest` en `apps/backend`: `test_phenoage.py`, `test_evolution.py`, `test_montecarlo.py`, `test_biological_age_router.py`). Siempre debe existir un **caso demo precargado** — nunca depender de upload/OCR en vivo durante el pitch.

Lo que explícitamente NO se construye (spec §12): foto envejecida, dieta/suplementos/alergias, genética, >3 intervenciones simultáneas, SHAP sobre las 10000 trayectorias.
...
- Bottom nav en los mockups: Futuro · Simular · Respaldo. En la app (decisión 2026-08-22) el bottom nav es Futuro · Simular · Pregúntame · Perfil y Respaldo se abre desde el pill de la esquina superior derecha de "Tu futuro" (pierde protagonismo a favor del chat).
