...
| `POST /me/health-context/montecarlo` | `{escenarios, n_trayectorias: 5000, anios: 10}`. Devuelve por escenario los percentiles al horizonte **y el objeto `curva`** con `{anios, p10, mediana, p90}` año por año. La app pide `ninguna`, `ejercicio_aerobico`, `dieta_mediterranea` siempre; `sueno_8h` si `sueno_h < 8`; `reducir_estres` si `estres` no es bajo; `cesacion_tabaco` + `combinada` solo si `tabaco == true`. El motor expone 7 escenarios (los 6 anteriores más los dos nuevos); la app pide solo los accionables porque el efecto por escenario es fijo y ofrecer "dormir 8 horas" a quien ya duerme 8 prometería años que no puede ganar. |
...
Cómo se arma el resultado de la spec §8 (`SimulationRepository._simularRemoto`): `edad_biologica_hoy` = phenoage; `trayectoria_baseline` = **el objeto `curva` del escenario `ninguna` tal cual** (`Curva.fromJson`), sin interpolar; cada escenario → `anios_ganados` = mediana base − mediana escenario, `rango` ≈ ±1,28·sd (sd de la banda, pareado ×0,5), `pct_futuros_que_mejoran` ≈ Φ(delta/sd), `esfuerzo` de una tabla local (3/3/4/10), `ratio` = ganados/esfuerzo; `shap_top_drivers` y `comparacion_poblacional` son aproximaciones locales sobre `valores_usados`; `muestra_trayectorias` son líneas ilustrativas coherentes con la banda.

## Lo que la app resuelve en local porque el backend aún no lo expone

1. ~~**Curva por año**~~ — **resuelto**. `/montecarlo` devuelve `curva {anios, p10, mediana, p90}` por escenario y la app la consume directo. Queda como fallback `_curvaAproximada()` (mediana lineal, banda ∝ √t) solo por si un APK viejo habla con un backend anterior. Sigue faltando: **`muestra_trayectorias`** (40–80 líneas individuales por escenario — el backend solo expone percentiles, así que las que dibuja el abanico se sintetizan en el dispositivo) y **`pct_futuros_que_mejoran`** pareado por semilla (hoy ≈ Φ(delta/sd) en local).
2. **SHAP / "por qué"** y **percentil poblacional NHANES** (spec §7/§8): hoy aproximados en el dispositivo.
3. **Catálogo de escenarios y esfuerzo**: el motor ya trae `sueno_8h` y `reducir_estres` (spec §5); siguen faltando **alcohol** y las **combinaciones de 1–3 palancas** por barrido (hoy `combinada` es una sola combinación precocinada en el backend). El **esfuerzo** por escenario sigue siendo una tabla local en la app (`escenariosBackend`, 2/3/3/2/4/10) — el backend no lo expone, así que el ranking por ratio impacto/esfuerzo se calcula en el dispositivo.
